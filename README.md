#系统已知缺陷:
1.连接字符串IP地址可能出现变动导致系统无法打开，甚至坏死在后台。
2.多处重复代码：需要改善
3. 代码更新方向:在Bll层添加各模块的逻辑代码类，继承并实现ModelExecuteiionOption类，在此层次结构中，完成一切逻辑代码。从而是UI层做到只显示，只调用执行!!!!!!->二次整改立即使用
...
+ **Select_TeachsPagration：采用模糊查询+分页的形式查询教师信息**
